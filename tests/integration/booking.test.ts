...
describe('GET /booking', () => {
  ...
  describe('when token is valid', () => {
    it('should respond status 404 if user does not have a booking', async () => {
      ...
      const result = await server.get('/booking').set('Authorization', `Bearer ${token}`).send({ roomId: room.id });

      expect(result.status).toEqual(httpStatus.NOT_FOUND);
    });

    it('should respond status 200 and reservation data when user has a booking', async () => {
      const user = await createUser();
      const token = await generateValidToken(user);
